...
bool BMPImage::load(const std::string& filename)
{
    ...
    width = read_s32_le(&dibHeader[4]);
    int32_t rawHeight = read_s32_le(&dibHeader[8]);
    height = std::abs(rawHeight);
    originalTopDown = (rawHeight < 0);

    uint16_t planes = read_u16_le(&dibHeader[12]);
    uint16_t bpp = read_u16_le(&dibHeader[14]);
    uint32_t compression = read_u32_le(&dibHeader[16]);

    if (planes != 1 || bpp != BYTES_PER_PIXEL_24 * 8 || compression != 0)
        throw std::runtime_error("Only uncompressed 24bpp BMP supported");

    uint32_t pixelOffset = read_u32_le(&bmpHeader[10]);
    ...
    if (originalTopDown) {
        for (int y = 0; y < height / 2; ++y) {
            std::swap_ranges(
                pixelData.begin() + y * rowSize,
                pixelData.begin() + (y + 1) * rowSize,
                pixelData.begin() + (height - 1 - y) * rowSize
            );
        }
    }
